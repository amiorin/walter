...
- The launcher has no `pin` command, by design — pinning is a maintainer task inside the `walter` repository, because it reads the HEAD of whatever git repository surrounds it. If the launcher reports a contract mismatch, re-copy `green` from an updated skill; never try to repin it in place.
